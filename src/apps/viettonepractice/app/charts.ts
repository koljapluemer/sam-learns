// Port of linguanodon's viettonepractice app/charts.js, replacing the CDN
// window.Chart/window.ChartErrorBars globals with real chart.js/
// chartjs-chart-error-bars npm imports (same library, proper module import
// instead of a <script> tag - see docs/linguanodon-import.md).
import {
  BarController,
  BarElement,
  CategoryScale,
...
export function createAccuracyTrendChart(canvas: ChartItem, initialTrials: AccuracyTrialPoint[]) {
...
export function createDailyVolumeChart(canvas: ChartItem, days: DailyExercisePoint[]) {
...
export function createDailyAccuracyChart(canvas: ChartItem, days: DailyAccuracyPoint[]) {
  const palette = readChartPalette()
...
export function createPairHistoryChart(canvas: ChartItem, trials: PairAccuracyTrialPoint[]) {
  const palette = readChartPalette()
