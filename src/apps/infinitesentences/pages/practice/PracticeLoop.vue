...
// Port of linguanodon's infinitesentences app/practiceApp.js. The language
// pair is chosen once in the setup modal (see PagePractice.vue) and passed
// in as props; this component owns the actual practice session.
...
const props = defineProps<{ nativeIso: string; targetIso: string }>()

const session = usePracticeSession(props.nativeIso, props.targetIso)
useActiveTime('infinitesentences')

const nativeLabel = ref(props.nativeIso)
const targetLabel = ref(props.targetIso)
...
onMounted(async () => {
  ...
  try {
    ...
    const native = languages.find((language) => language.code === props.nativeIso)
    const target = languages.find((language) => language.code === props.targetIso)
