...
import LicenseModal from '../../dumb/LicenseModal.vue'
import { getLanguage } from '../../entities/tone-clip/languages/registry'
import { usePracticeSession } from '../../features/practice-session/usePracticeSession'

const props = defineProps<{
  languageCode: string
  hanziVisibility: 'always' | 'after-answer' | 'never'
}>()

// Guaranteed present: languageCode only ever comes from listLanguageOptions().
const languagePack = getLanguage(props.languageCode)!
...
const {
  ...
  round,
  splitLabel
} = usePracticeSession(languagePack)

const licenseModalOpen = ref(false)
const hasAnswered = computed(() => phase.value === 'wrong' || disabledButtonIndex.value !== null)
const showNativeScript = computed(() => {
  if (!round.value?.clip.nativeScript) return false
  if (props.hanziVisibility === 'never') return false
  if (props.hanziVisibility === 'always') return true
  return hasAnswered.value
})
</script>

<template>
  <section class="mx-auto flex w-full max-w-4xl flex-1 flex-col items-center justify-center gap-4 p-4">
        ...
        >
          <div class="card border border-base-300 bg-base-200">
            <div class="card-body gap-4 p-4">
              ...
              <p
                v-if="showNativeScript"
                class="text-center text-lg"
              >
                {{ round.clip.nativeScript }}
              </p>
            </div>
          </div>
    ...
    <p class="text-center text-sm text-base-content/70">
      ...
      <a
        class="link"
        :href="languagePack.credits.datasetUrl"
        target="_blank"
        rel="noopener"
      >{{ languagePack.credits.datasetName }}</a>.
      <button
        type="button"
        class="link ml-1"
    ...
    <LicenseModal
      ...
      :license-name="languagePack.credits.licenseName"
      :license-url="languagePack.credits.licenseUrl"
      :license-summary="languagePack.credits.licenseSummary"
