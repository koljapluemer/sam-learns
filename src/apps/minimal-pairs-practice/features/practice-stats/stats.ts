// Pure port of linguanodon's viettonepractice app/stats.js (itself a port of
// listen-to-viet's src/entities/practice-progress/stats.ts), unchanged
// except getPracticeStatsSnapshot now takes the active language's tone-key
// set as a parameter instead of importing Vietnamese's fixed TONE_KEYS.
import type { DistractorCandidate, PracticePairTarget } from '../../entities/tone-clip/types'
...
function toBidirectionalPracticePairTarget(leftKey: string, rightKey: string): PracticePairTarget {
  const [firstKey, secondKey] = sortPairKeys(leftKey, rightKey)
  return { correctKey: firstKey, distractorKey: secondKey }
}

function parsePairKey(pairKey: string): PracticePairTarget | null {
...
function getEventPairTarget(event: PracticeEvent): PracticePairTarget | null {
  return event.correctTone && event.distractorTone
    ? toBidirectionalPracticePairTarget(event.correctTone, event.distractorTone)
    : null
}

function getDirectionalEventPairTarget(event: PracticeEvent): PracticePairTarget | null {
  return event.correctTone && event.distractorTone ? { correctKey: event.correctTone, distractorKey: event.distractorTone } : null
}

export function getCandidatePairTarget(candidate: DistractorCandidate): PracticePairTarget {
  return toBidirectionalPracticePairTarget(candidate.correctTone, candidate.distractorTone)
}

export function getDirectionalCandidatePairTarget(candidate: DistractorCandidate): PracticePairTarget {
  return { correctKey: candidate.correctTone, distractorKey: candidate.distractorTone }
}
...
function toDecayedPairHistoryStats(pair: PairCounts): DecayedPairHistoryStats {
  return {
...
export function getBidirectionalDecayedPairHistory(events: PracticeEvent[]): Map<string, DecayedPairHistoryStats> {
...
export function getDirectionalDecayedPairHistory(events: PracticeEvent[]): Map<string, DecayedPairHistoryStats> {
...
function getDailyExerciseSeries(events: PracticeEvent[]): DailyExercisePoint[] {
...
function getWilsonConfidenceInterval95(correct: number, trials: number): { high: number | null; low: number | null } {
...
function getDailyAccuracySeries(events: PracticeEvent[]): DailyAccuracyPoint[] {
...
function getEmptyMatrixCell(correctKey: string, distractorKey: string): MatrixCellStats {
  return {
    correctKey,
    distractorKey,
    attempts: 0,
...
export function getPracticeStatsSnapshot(events: PracticeEvent[], toneKeys: readonly string[]): PracticeStatsSnapshot {
  ...
  return {
    ...
    tone: toMatrixSummary(toneKeys, buildDirectionalPairCounts(trackedEvents))
  }
}

export function getAccuracyTrialSeries(events: PracticeEvent[], visibleWindow?: number): AccuracyTrialPoint[] {
...
export function getPairAccuracyTrialSeries(events: PracticeEvent[], pairTarget: PracticePairTarget): PairAccuracyTrialPoint[] {
