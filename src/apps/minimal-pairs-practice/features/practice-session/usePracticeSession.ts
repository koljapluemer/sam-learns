// Vue composable port of linguanodon's viettonepractice app/session.js
// (itself a near-verbatim port of listen-to-viet's usePracticeSession.ts).
// No pairFilter/metaMode concept - those only existed to serve the dropped
// pair-practice page. queueEvent(...) is replaced with logActivity(...);
// there is no queueState/server sync. Takes a LanguagePack instead of raw
// {audioBaseUrl, apiClipsUrl} so it works identically for any language pack
// registered in entities/tone-clip/languages/registry.ts.
import { computed, nextTick, onMounted, onUnmounted, ref } from 'vue'
import { logActiveTimeMs, logActivity } from '@/shared/activity/useLearningEvent'
import { buildPracticeCatalog, toClips } from '../../entities/tone-clip/catalog'
import type { LanguagePack } from '../../entities/tone-clip/languages/types'
import type { AnswerOption, Clip, DistractorCandidate, PracticeCatalogEntry } from '../../entities/tone-clip/types'
import { appendPracticeEvent, listPracticeEvents, toPracticeEventAnalytics, toStoredClip } from '../../entities/practice-event/practiceEvents'
...
} from '../practice-stats/stats'
import type { DecayedPairHistoryStats } from '../practice-stats/types'
...
const ACTIVITY_APP_SLUG = 'minimal-pairs-practice'
...
type Round = {
  clip: Clip
  candidate: DistractorCandidate
  selectionMode: 'strategyA' | 'strategyB'
...
function getStrategyBScore(pairHistory: DecayedPairHistoryStats | undefined): number {
  return !pairHistory ? -1 : pairHistory.decayedPosteriorAccuracy
}

export function usePracticeSession(languagePack: LanguagePack) {
  ...
  const currentListeningSelectionMode = ref<'strategyA' | 'strategyB' | undefined>(undefined)
  ...
  const syncAvailableClips = () => {
    clips.value = clipCatalog
      .filter((entry) => !hiddenClipFilenames.value.has(entry.clip.filename))
  ...
  const getExercises = (entries: PracticeCatalogEntry[]): Exercise[] =>
  ...
  const groupExercisesByPair = (exercises: Exercise[]): Map<string, Map<string, Exercise[]>> => {
    const groups = new Map<string, Map<string, Exercise[]>>()
    exercises.forEach((exercise) => {
      ...
      const directions = groups.get(pairKey) ?? new Map<string, Exercise[]>()
  ...
  const createRound = (exercise: Exercise, selectionMode: 'strategyA' | 'strategyB'): Round => ({
    clip: exercise.clip,
  ...
  const chooseStrategyAExercise = (
    pairExercises: Map<string, Map<string, Exercise[]>>,
  ...
  const chooseDirectionKey = (
    directionalExercises: Map<string, Exercise[]>,
  ...
  const generateNextRound = (): Round | null => {
    ...
    for (let attempt = 0; attempt < MAX_ROUND_GENERATION_ATTEMPTS; attempt += 1) {
      ...
      const selectionMode: 'strategyA' | 'strategyB' = useStrategyB ? 'strategyB' : 'strategyA'
  ...
  const finalizeAudioPlaybackTracking = async () => {
    ...
    void logActiveTimeMs(ACTIVITY_APP_SLUG, listenedDurationMs)
  }
  ...
  const handleAnswer = async (option: AnswerOption, buttonIndex: number) => {
    ...
    if (phase.value === 'ready') {
      ...
      void logActivity(ACTIVITY_APP_SLUG)
    }
  ...
  const loadSessionState = async () => {
    const clipsResponse = await fetch(languagePack.clipsUrl)
    if (!clipsResponse.ok) throw new Error('Could not load clip data.')

    const rawClips = (await clipsResponse.json()) as { filename: string; transcript: string; nativeScript?: string }[]
    const parsedClips = toClips(rawClips, languagePack.audioBaseUrl)
    clipCatalog = buildPracticeCatalog(parsedClips, languagePack.generateDistractors)
    practiceEvents.value = await listPracticeEvents(languagePack.code)
    syncHiddenClipsFromEvents()
  }
