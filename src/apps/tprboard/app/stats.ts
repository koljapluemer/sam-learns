...
export function createStatsTracker(): StatsTracker {
  ...
  const flushPlayedTime = (now: number = Date.now()) => {
  ...
  return {
    ...
    recordCompletedTask: (wasPerfectRound: boolean) => {
      flushPlayedTime()
...
export function formatPlayedTime(timePlayedMs: number): string {
...
export function readPlayerStats(): PlayerStats {
  ...
  try {
    ...
    const recordStreakBaseline =
      currentStreak === 0 ? bestStreak : Math.min(hasStoredRecordBaseline ? rawRecordBaseline : bestStreak, bestStreak)
