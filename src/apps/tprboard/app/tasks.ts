// Port of linguanodon's tprboard app/tasks.js - the round-planning/difficulty
// algorithm. Pure functions/data, no DOM - logic kept as-is, only JSDoc->TS.
...
export function createRelationshipIndex(objectPool: PlacedObject[], localeTaskMap: LocaleTaskMap): RelationshipIndex {
  ...
  objectPool.forEach(({ name, record }) => {
    ...
    Object.entries(record.relationships ?? {}).forEach(([targetName, relationship]) => {
      ...
      const playableRelationship = findPlayableRelationship(name, targetName, relationship, localeTaskMap)
...
function chooseRandomTask(relationshipIndex: RelationshipIndex): TaskCandidate {
...
function isCorrectAction(sourceName: string, targetName: string, correctTask: TaskCandidate): boolean {
  return sourceName === correctTask.sourceName && targetName === correctTask.targetName
}

export function calculateBoardDifficulty(placedObjects: PlacedObject[], correctTask: TaskCandidate): DifficultyBreakdown {
  ...
  const overlappingRelationshipDifficulty = overlappingRelationshipCount * OVERLAPPING_RELATIONSHIP_DIFFICULTY
  const unrelatedRelationshipDifficulty = unrelatedRelationshipCount * UNRELATED_RELATIONSHIP_DIFFICULTY
  ...
  return {
    ...
    total: missingRelationshipDifficulty + overlappingRelationshipDifficulty + unrelatedRelationshipDifficulty,
...
function resolveDifficultyRule(
  ...
  if (!languageProgress) {
    return {
      kind: 'floor',
      reason:
        'Both task objects have been seen, but there is no previous round for this language yet, so start from a floor of 0.',
      value: 0
    }
  }
...
function chooseBestSimulation(simulations: CandidateSimulation[], difficultyRule: DifficultyTarget): CandidateSimulation | null {
...
function hasSatisfiedDifficultyRule(difficulty: number, difficultyRule: DifficultyTarget): boolean {
  return difficultyRule.kind === 'ceiling' ? difficulty < difficultyRule.value : difficulty > difficultyRule.value
}
...
function buildRoundPlan(
  ...
  const difficultyTarget = resolveDifficultyRule(selection.activeTask, learningItemsByObjectName, languageProgress)
  ...
  while (placedObjects.length < BOARD_CAPACITY) {
    ...
    const simulations = shuffled(candidateNames)
      .map((candidateName) => simulateCandidate(candidateName, placedObjects, relationshipIndex, selection.activeTask))
...
export function planRound({ languageProgress, learningItemsByObjectName, relationshipIndex, sentenceItemsByKey }: PlanRoundOptions): RoundPlan {
...
export function objectHasRelationships(record: ObjectRecord): boolean {
